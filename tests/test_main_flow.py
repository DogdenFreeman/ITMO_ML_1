...
def test_topup_balance(client: TestClient, auth_token: str):
    ...
    assert pytest.approx(data_topup["balance"]) == balance_before + topup_amount
    ...
    topup_found = any(
        t["transaction_type"] == "topup" and pytest.approx(t["amount"]) == topup_amount
...
def test_create_prediction_success(client: TestClient, auth_token: str):
    ...
    assert pytest.approx(data_predict["cost"]) == prediction_cost
    ...
    assert pytest.approx(balance_after) == balance_before - prediction_cost
    ...
    transactions = response_history.json()
    fee_found = any(
        t["transaction_type"] == "prediction_fee" and pytest.approx(t["amount"]) == -prediction_cost
        for t in transactions
    )
